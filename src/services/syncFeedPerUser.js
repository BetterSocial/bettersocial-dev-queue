...
const requestStream = async (feedType, clientFeed, offset, limit) => {
  if (feedType == 'following') {
    return await clientFeed.following({offset, limit});
  }
  if (feedType == 'follower') {
    return await clientFeed.followers({offset, limit});
  }
  return await clientFeed.get({offset, limit});
};
...
const resetAndSyncFeed = async (req, res) => {
  try {
    ...
    await Promise.all(
      followinfFeed.map(async (feed) => {
        const targetFeed = feed.target_id.split(':');
