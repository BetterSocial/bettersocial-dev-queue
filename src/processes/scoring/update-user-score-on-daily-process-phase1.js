...
const updateUserScoreOnDailyProcessPhase1 = async(
        job, userScoreCol, processTime, userIds, lastBatch) => {
  ...
  while (await cursors.hasNext()) {
    ...
    userDoc.r_score = await averagePostScore(totalPostScores, lastp3Scores._count);
...
function sendQueue(processTime) {
  ...
  scoringProcessQueue.sendQueueForDailyProcess(EVENT_DAILY_PROCESS_USER_SCORE_PHASE2, scoringProcessData);
}
