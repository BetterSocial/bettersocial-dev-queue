...
function updateLastBlocks(lastBlocks, activityTime) {
  ...
  if (!isStringBlankOrNull(lastBlocks.last_update)) {
    const dayDiffLastUpdateAndPostTime = moment.duration(momentActivityTime.diff(moment.utc(lastBlocks.last_update, REGULAR_TIME_FORMAT, true))).as('days');
    ...
    if (dayDiffLastUpdateAndPostTime >= 0 && dayDiffLastUpdateAndPostTime <= 1) {
      console.debug("calcScoreOnUnblockUser:updateLastBlocks -> last_update is earlier from activity time and less than a day");
      ...
      if (!isStringBlankOrNull(lastBlocks.earliest_time)) {
        const dayDiffEarliestTimeAndActivityTime = moment.duration(momentActivityTime.diff(moment.utc(lastBlocks.earliest_time, REGULAR_TIME_FORMAT, true))).as('days');

        if (dayDiffEarliestTimeAndActivityTime <= 7) {
          console.debug("calcScoreOnUnblockUser:updateLastBlocks -> earliest_time is less than or equals 7 days earlier from activity time");

          lastBlocks.last_update = activityTime;
          lastBlocks.counter = lastBlocks.counter - 1;
        }
      }
    }
  }
 ...
 */
const calcScoreOnUnblockUserPost = async(data, userScoreDoc, blockedUserScoreDoc, userScoreList) => {
  ...
  if (userScoreDoc.blocking && userScoreDoc.blocking.indexOf(blockedUserScoreDoc._id) > -1) {
    console.debug("User " + blockedUserScoreDoc._id + " already blocked by user " + userScoreDoc._id);
    ...
    // 3. Add the author in blocking list
    userScoreDoc.blocking.push(blockedUserScoreDoc._id);
    ...
    console.debug("Skipping unblock user " + blockedUserScoreDoc._id + " as blocked by user " + userScoreDoc._id);
  }
}
 ...
 */
async function calcScoreOnBlockPost(data, userScoreDoc, blockedUserScoreDoc, userScoreList, postScoreDoc, postScoreList, userPostScoreDoc, userPostScoreList) {
  ...
  if (existingActivityLog) {
    console.debug("calcScoreOnBlockUserPost:calcScoreOnBlockPost -> skip block process since it already exists in the log : " + existingActivityLog);
  } else {
    ...
    if (userPostScoreDoc.block_count > 0) {
      ...
      if (userPostScoreDoc.anomaly_activities.block_time === "" ||
        moment.utc(data.activity_time).diff(
          moment.utc(userPostScoreDoc.anomaly_activities.block_time), 'seconds') > 0) {
      ...
      if (!postScoreDoc.anonimity) {
        ...
        if (userScoreDoc.blocking && userScoreDoc.blocking.indexOf(blockedUserScoreDoc._id) > -1) {
          ...
          console.debug("Adding user " + blockedUserScoreDoc._id + " as blocked by user " + userScoreDoc._id);
      ...
      const blockPoint = postInteractionPoint(userScoreDoc.last_blocks.counter, B_REC);
...
const calcScoreOnUnblockUser = async(data, userScoreDoc, blockedUserScoreDoc, userScoreList, userPostScoreList) => {
  ...
  if (!postScoreDoc) {
    ...
    result = await calcScoreOnBlockPost(data, userScoreDoc, blockedUserScoreDoc, userScoreList, postScoreDoc, postScoreList, userPostScoreDoc, userPostScoreList);
  }
